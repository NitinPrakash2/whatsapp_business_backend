...
export default async function(request: FastifyRequest, reply: FastifyReply, /*done: HookHandlerDoneFunction*/ ) {
    ...
    //set..
    let _token_data:any = _verify_token;
    ...
    //vali..
    if (_token_data[`security`][`party`].indexOf(`party_2`)==-1) {
      //err..
      send_err();
      return;
    }
    if (url.startsWith('/client/api/i/')) {
    const instance = params?.instance;
    const instanceId = validate_uuid(instance) ? instance: undefined;
    //console.log(instance);
    //console.log(instanceId);
    if (_token_data[`security`][`route`][`i`].indexOf(`*`)==-1 && _token_data[`security`][`route`][`i`].indexOf(instanceId)==-1) {
      //err..
      send_err();
      return;
    }
    }
    if (url.startsWith('/client/api/i_init/')) {
    const instance = params?.instance;
    const instanceId = validate_uuid(instance) ? instance: undefined;
    //console.log(instance);
    //console.log(instanceId);
    if (_token_data[`security`][`route`][`i_init`].indexOf(`*`)==-1 && _token_data[`security`][`route`][`i_init`].indexOf(instanceId)==-1) {
      //err..
      send_err();
      return;
    }
